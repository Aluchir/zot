...
func (is *ImageStore) FinishBlobUpload(repo, uuid string, body io.Reader, dstDigest godigest.Digest) error {
	...
	// The repo lock cannot be taken here either (it is always acquired before the
	// store lock), so the index write is unfenced; it only ever creates an index
	// that is absent, never rewrites one.
...
func (is *ImageStore) FullBlobUpload(ctx context.Context, repo string, body io.Reader,
	...
	// unfenced: this runs under the store lock, which is taken after the repo
	// lock, so the fence is unavailable here; the write only creates an absent index
