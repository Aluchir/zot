...
# change up anyway (directory watch plus stat polling) and apply the reloadable
# subset without a pod restart.
#
# This test:
...
# Capture the logs once: piping kubectl logs into grep -q trips pipefail via
# SIGPIPE when grep exits on the first match.
ZOT_LOGS=$(kubectl --context "${KUBECTL_CTX}" logs "${POD}")

log_info "zot logs (reload lines):"
echo "${ZOT_LOGS}" | grep -E "config file changed|reloaded params" || true

if [ "${RELOADED}" != "1" ]; then
    log_error "bob still denied after ${RELOAD_TIMEOUT_SECONDS}s; config reload did not happen"
    echo "${ZOT_LOGS}" | tail -40
    exit 1
fi

echo "${ZOT_LOGS}" | grep -q "config file changed" || {
